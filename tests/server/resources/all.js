...
(function() {
    ...
    if (window.implemented) {
        for (impl in implemented) {
            nodes = doc.getElementsByClassName ? doc.getElementsByClassName(impl) :
            (function () {
                var lis = doc.getElementsByTagName("li");
                var ret = [];
                for (var i=0;i<lis.length;++i) {
                    if (lis[i].className.indexOf(impl) > -1)
                        ret.push(lis[i]);
                }
                return ret;
            })();
    ...
    function selectFramework() {
        var qstr = '?framework=' + doc.getElementById('select-framework').value;
    ...
    function runTests() {
        doc.getElementById('run-tests').disabled = true;

        globalTimeout = window.setTimeout(function() {
            failed = true;
            travisResult.innerHTML = 'fail';
            doc.body.appendChild(travisResult);
        }, globalAMDTimeout);
        ...
        nodes = doc.getElementById('tests').getElementsByTagName('a');
        failed = false;
  ...
  document.getElementById('run-tests').removeAttribute('disabled');
    ...
    doc.getElementById('select-framework').onchange = selectFramework;
    doc.getElementById('run-tests').onclick = runTests;
